...
int isValidID(char * lex){
	if (strlen(lex) == 0) return 0;
	for (int i =0 ; i<strlen(lex); i++){
		if (!(isalpha(lex[i])!=0 || lex[i] == '_' || (lex[i] - '0'>=0 && lex[i]-'0'<=9 && i>=1))){
			return 0;
		}
	}
	return 1;
    ...
    for (int i = 0; i<curr_ind; i++){
        ...
        if (((key = isKeyword(symbol_table[i].lex))>=0)){
            ...
            if (strcmp(symbol_table[i+1].lex,"(")==0){
            	...
            	if(strcmp(symbol_table[i].lex,"*")==0){
            		i++;
            	}
            	if(!isValidID(symbol_table[i].lex)) continue;
